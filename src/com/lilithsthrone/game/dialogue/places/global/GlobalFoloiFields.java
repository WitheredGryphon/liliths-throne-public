...
//						AbstractPlaceType pt = ((AbstractGlobalPlaceType) Main.game.getPlayer().getGlobalCell().getPlace().getPlaceType()).getGlobalLinkedWorldType().getEntryFromGlobalMapLocation();
	...
	public static final DialogueNode ILLETON_EXTERIOR = new DialogueNode("Illeton", "", false) {
		@Override
		public int getSecondsPassed() {
			return 60 * 60;
		}
		...
		@Override
		public String getContent() {
			return UtilText.parseFromXMLFile("places/global/globalPlaces", "ILLETON_EXTERIOR");
		}

		@Override
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new ResponseEffectsOnly("Enter", "Head to Illeton's gates.") {
					@Override
					public void effects() {
//						WorldType wt = ((AbstractGlobalPlaceType) Main.game.getPlayer().getGlobalCell().getPlace().getPlaceType()).getGlobalLinkedWorldType();
//						AbstractPlaceType pt = ((AbstractGlobalPlaceType) Main.game.getPlayer().getGlobalCell().getPlace().getPlaceType()).getGlobalLinkedWorldType().getEntryFromGlobalMapLocation();
						Main.game.getPlayer().setLocation(
								WorldType.ILLETON,
								PlaceType.ILLETON_GATE,//Main.game.getWorlds().get(wt).getCell(pt).getLocation(),
								false);
